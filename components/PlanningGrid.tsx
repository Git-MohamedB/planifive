          ...
          {unsavedChanges && (
            <div className="absolute left-1/2 top-0 -translate-x-1/2 z-10 h-full">
              <ActionButton
                ...
                icon={<Save size={14} />}
                green
                className="rounded-full cursor-pointer shadow-[0_0_15px_rgba(34,197,94,0.3)] px-8"
              />
            </div>
          )}
                ...
                {HOURS.map((hour) => (
                  <Fragment key={hour}>
                    ...
                    {DAYS.map((_, i) => {
                      ...
                      // Dynamic Styles
                      let parentClasses = `relative z-0 group transition-all duration-200 border-b border-r border-[#222] cursor-pointer flex flex-col items-center justify-center group-hover:z-50`;
